...
- Show a short description of what the band means and a validation note with every result, in the chat and in the Mini App alike. Both come from `src/instruments.mjs`, through `bandOf` and each scale's `validation`. GAD-7 and PHQ-9 cite their original studies: Spitzer 2006 (α 0.92; at 10, sensitivity 89% and specificity 82%) and Kroenke 2001 (α 0.86 to 0.89; at 10, 88% and 88%). The note says these figures describe the English originals, because the bot's wording is a working translation. The own scales say no validation was done. When the PHQ-9 risk item is marked below the cutoff, `interpretResult` replaces the reassuring band text and the "keep watching" advice with wording that points at the answer that matters. The crisis block now comes before the reference material. The Mini App result screen also gains the own-scale notice it was missing.

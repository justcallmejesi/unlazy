...
- Route `/paysupport` to the owner: a button opens `ADMIN_USERNAME` with a draft holding the user id and charge ids. The old text said "write here", and the bot forwards nothing.
- A 403 from one chat no longer drops the actions for other chats in the batch, such as a second specialist's risk alert, and no longer recreates a deleted record.
- `/results`, `/last` and `/clients` split into several messages instead of failing past 4096 characters. `/export` sends a complete JSON file when it outgrows a message instead of a clipped one.
- Starting a questionnaire, a booking or a specialist application ends whichever other flow was open, so a forgotten note or application can no longer swallow a typed answer. `/cancel` also ends an application.
- Reminder settings from the Mini App are behind the same paid check as `/remind`. The app asks before wiping its own copy of the data. An emoji cut in half no longer breaks the booking link. An empty `REMINDER_WEEKDAY` means Monday, not Sunday.
- Threshold alerts for the bot's own scales carry the self-observation caveat. The off-machine backup reads its SSH key from the state directory, the only place its sandboxed unit can reach.
- Add a specialist cabinet sold as a monthly Stars subscription, 177 Stars by default (`PSY_PRICE_STARS`). The link comes from `createInvoiceLink` with `subscription_period`, Telegram renews it, and auto-renewal toggles through `editUserStarSubscription` with the first charge id. Specialists apply with a name, credentials and acceptance of terms. The owner approves or rejects each application, bound by `ADMIN_USERNAME` on first contact or pinned by `ADMIN_CHAT_ID`, and uses the cabinet free. An approved, subscribed specialist gets an invite link, a client list with scores and trends, a client card, a report per client, and alerts when a client crosses a cutoff or marks PHQ-9 item 9. Every read checks approval, subscription and that client's consent to that specialist.
- Add a versioned consent form for clients. It says what the bot collects, what the specialist sees and does not see, why, how to withdraw, and the legal basis. Nothing is shared before "Погоджуюся", notes need a separate yes, and `/privacy` lists every consent with one-tap revocation and a notes toggle. Revocations are kept as a record, and the specialist is told at once. `/delete` notifies specialists, and a specialist's `/delete` closes every consent given to them.
- Subscribe to `pre_checkout_query` in `getUpdates`. Without it Telegram never delivered the query, nothing answered within ten seconds, and every Stars payment, including the one-time purchase, would have failed at the last step.
- Split payments by payload, so a subscription payment can never be read as the client purchase. Correct `/paysupport`, which listed the weekly reminder among the free features after a refund.
- Let a router action carry `then`, a function of the call's result returning follow-up actions, so the invoice link reaches the chat while the router stays free of I/O.
